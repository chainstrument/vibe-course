...
export default function ExercicesPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-16">
      <h1 className="text-3xl font-bold text-zinc-900 mb-2 dark:text-white">Exercices pratiques</h1>
        ...
        <div className="flex flex-col gap-3">
          {exercises.map((ex) => (
            <Link
              ...
              className="group flex items-start justify-between gap-4 rounded-xl border border-zinc-100 bg-white p-6 shadow-sm hover:shadow-md hover:border-indigo-200 hover:bg-indigo-50/30 transition-all dark:border-white/10 dark:bg-white/5 dark:shadow-none dark:hover:bg-white/[0.08] dark:hover:border-indigo-500/30"
            >
              <div>
              ...
              {ex.level && (
                <span className={`flex-none text-xs font-medium px-2.5 py-1 rounded-full ${levelColors[ex.level] ?? "bg-zinc-50 text-zinc-600 ring-1 ring-zinc-200"}`}>
                  {ex.level}
                </span>
              )}
            </Link>
